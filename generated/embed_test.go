...
func TestResolvedSpecBytes(t *testing.T) {
	...
	// The repo may intentionally contain the committed placeholder `{}` before
	// `go generate ./...` has been run. After generation, the embedded spec must
	// have the normalized top-level shape.
	if len(got) == 0 {
		return
	}
