...
const defaultEndpoint = "https://api.thousandeyes.com/v7"
...
func (c ConnectionConfig) NewAPICaller(_ context.Context, _ time.Duration, httpClient *http.Client) implementations.APICaller {
	if !c.HasToken() {
		return unavailableAPICaller{err: contracts.AuthError{Message: "ThousandEyes bearer token is not configured; search is available, but query and mutate require THOUSANDEYES_BEARER_TOKEN or THOUSANDEYES_API_TOKEN"}}
	}
...
func LoadConnectionConfig(args []string, environ []string) (ConnectionConfig, error) {
	...
	fs.StringVar(&endpointFlag, "endpoint", "", "base ThousandEyes API endpoint")
	...
	cfg.Token = strings.TrimSpace(env["THOUSANDEYES_BEARER_TOKEN"])
	if cfg.Token == "" {
		cfg.Token = strings.TrimSpace(env["THOUSANDEYES_API_TOKEN"])
	}

	endpointRaw := defaultEndpoint
	if value := strings.TrimSpace(env["THOUSANDEYES_ENDPOINT"]); value != "" {
		endpointRaw = value
	}
	...
	proxyRaw := env["THOUSANDEYES_PROXY_URL"]
	if setFlags["proxy"] {
		proxyRaw = proxyFlag
	}
